...
$obj = new User();
$obj->setData($_GET);
$adminIndex = $obj->admin_index();
$object = new User();
$object->setData($_GET);
$adminViewIndex = $object->admin_view_index();
$chamber = new chamber();
...
<h4>Item found : <?php echo count($adminIndex)?></h4>
        <div class="nav navbar-default" style="margin: 15px;margin-bottom:15px;padding: 15px">
            <a class="btn btn-danger" href= "Admin/Authentication/logout.php" > LOGOUT </a>
            <a class="btn btn-primary" href='admintrashed.php'>Trashed List</a>
            <button id="TrashSelected" class='btn bg-primary'>Trash Selected</button>
            <button id="DeleteSelected" class='btn btn-danger'>Delete Selected</button>
            <a class='btn btn-info' href="Admin/profile/create.php">Create Another Admin</a>
        </div>
        ...
        foreach ($adminIndex as $adminViewIndex){
            ...
            <div>
             <a href='Admin/profile/delete.php?id=$adminViewIndex->id' class='btn btn-primary' onclick='return warning()'>Delete User</a>
            <a class='btn btn-primary' href='Admin/profile/trash.php?id=$adminViewIndex->id'>Trash User</a>
            <a class='btn btn-primary' href='Admin/profile/email.php?id=$adminViewIndex->id'>Email User</a>
            </div>
        ...
        <div align="left" class="container">
            <ul class="pagination">
                ...
                if($page>$pages) Utility::redirect("adminindex.php?Page=$pages");

                if($page>1)  echo "<li><a href='adminindex.php?Page=$pageMinusOne'>" . "Previous" . "</a></li>";
                for($i=1;$i<=$pages;$i++)
                {
                    if($i==$page) echo '<li class="active"><a href="">'. $i . '</a></li>';
                    else  echo "<li><a href='adminindex.php?Page=$i'>". $i . '</a></li>';

                }
                if($page<$pages) echo "<li><a href='adminindex.php?Page=$pagePlusOne'>" . "Next" . "</a></li>";
...
//check "select all" if all checkbox items are checked
    ...
    $(document).ready(function () {
        ...
        $("#TrashSelected").click(function () {
            $("#multiple").attr('action', 'Admin/profile/trash_selected_doctor.php');
            $("#multiple").submit();
        });
